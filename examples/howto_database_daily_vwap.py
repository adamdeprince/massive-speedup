...
if len(sys.argv) != 3:
    raise SystemExit(f"usage: {sys.argv[0]} YYYY-MM-DD TICKER")
...
records = massive_speedup.StockTradeDatabase(date, ticker)
...
if aggregate is None or aggregate.volume == 0:
    raise SystemExit(f"no volume found for ticker {ticker!r} on {date}")
