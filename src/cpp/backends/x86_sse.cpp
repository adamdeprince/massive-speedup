...
namespace massive_speedup::backend_x86_sse {

struct Specialization {
  static inline std::size_t find_byte_sse(
    ...
    while (size - index >= chunk_size) {
      const __m128i block =
          _mm_loadu_si128(reinterpret_cast<const __m128i*>(data + index));
      const int mask = _mm_movemask_epi8(_mm_cmpeq_epi8(block, target));
      if (mask != 0) {
  ...
  static inline void split_csv_fields(
    ...
    while (size - index >= chunk_size) {
      ...
      int mask = _mm_movemask_epi8(
          _mm_or_si128(_mm_cmpeq_epi8(block, comma), _mm_cmpeq_epi8(block, quote)));
...
NB_MODULE(_sse, m) {
  massive_speedup::bindings::bind_backend_module<massive_speedup::backend_x86_sse::Implementation>(
      m,
      massive_speedup::BackendKind::x86_sse42,
      "_Sse");
}
